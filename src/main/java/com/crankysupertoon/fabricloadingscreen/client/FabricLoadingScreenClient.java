package com.crankysupertoon.fabricloadingscreen.client;

import com.crankysupertoon.fabricloadingscreen.FabricLoadingScreen;
...
public class FabricLoadingScreenClient implements ClientModInitializer {
    ...
    private static Integer getColorProperty(Properties properties, String key) {
        ...
        if (value != null) {
            try {
                ...
                FabricLoadingScreen.LOGGER.error("Invalid property value for " + key + " in client config. Expected hexadecimal RRGGBB.");
            }
        }
        return null;
    }
    ...
    private static int getIntProperty(Properties properties, String key, int def) {
        ...
        if (value != null) {
            try {
                ...
                FabricLoadingScreen.LOGGER.error("Invalid property value for " + key + " in client config. Expected integer.");
            }
        }
        return def;
    }
    ...
    private static Level getLogLevelProperty(Properties properties, String key, Level def) {
        ...
        if (value != null) {
            ...
            if (logLevel != null) {
                ...
                FabricLoadingScreen.LOGGER.error("Invalid property value for " + key + " in client config. Expected any of [" + StringUtils.join(Level.values(), ", ") + "].");
            }
        }
        return def;
    }
    ...
    public void onInitializeClient() {
        File configDirectory = new File(FabricLoader.getInstance().getConfigDirectory(), "loadingscreen");
        File clientConfig = new File(configDirectory, "client.config.properties");
        Properties properties = new Properties();
        if (configDirectory.exists() || configDirectory.mkdirs()) {
            if (!clientConfig.exists()) {
                try (BufferedReader br = new BufferedReader(new InputStreamReader(FabricLoadingScreen.class.getResourceAsStream("/client.config.properties")))) {
                    try (PrintWriter pw = new PrintWriter(new FileOutputStream(clientConfig))) {
                        ...
                        FabricLoadingScreen.LOGGER.error("Could not write default client config", e);
                    }
                } catch (IOException e) {
                    FabricLoadingScreen.LOGGER.error("Could not read default client config", e);
                }
            }
            ...
            try (InputStream in = new FileInputStream(clientConfig)) {
                ...
                FabricLoadingScreen.LOGGER.error("Could not read client config", e);
            }
        } else {
            FabricLoadingScreen.LOGGER.error("Could not create default client config, going with defaults");
        }
